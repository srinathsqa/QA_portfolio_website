...
const SingleProject = ({ name, year, align, image, link, description, tags, badge }) => {
    ...
    >
      ...
      <TechCard className="flex-1 max-w-md p-6" hover={true} glow={false}>
        <div className="space-y-4">
          ...
          {/* Badge */}
          {badge && (
            <div className={`flex ${align === "left" ? "md:justify-start" : "md:justify-start"} sm:justify-center`}>
              <div className="inline-block bg-yellow/20 border border-yellow/40 text-yellow text-xs font-bold px-3 py-1 rounded-full">
                {badge}
              </div>
            </div>
          )}
          ...
          {/* Description */}
          {description && (
            <p className="text-gray-300 text-sm leading-relaxed border-l-2 border-green/30 pl-3">
              {description}
            </p>
          )}
          ...
          {tags && (
            <div className="flex flex-wrap gap-2">
              {tags.map((tag, index) => (
                <span key={index} className="text-xs font-mono bg-green/10 text-green px-2 py-1 rounded">
                  {tag}
                </span>
              ))}
            </div>
