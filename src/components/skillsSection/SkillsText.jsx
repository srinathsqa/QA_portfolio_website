...
const SkillsText = ({ activeTab, setActiveTab }) => {
  return (
    <div className="flex flex-col items-center mt-[100px] relative">
      ...
      >
        <div className="flex items-center justify-center gap-2 mb-4">
          ...
          <span className="text-green font-mono text-sm">{'// QA Automation Tech Stack'}</span>
        </div>
        ...
        >
          {['Automation', 'Manual Testing', 'CI/CD & Tools', 'All'].map((tag, index) => (
            <button
              key={tag}
              onClick={() => setActiveTab(tag)}
              className={`px-3 py-1 bg-gray-800/50 border ${activeTab === tag ? 'border-green bg-green/20' : 'border-green/20'} rounded-full text-sm text-green font-mono hover:bg-green/20 transition-colors duration-300 cursor-pointer outline-none`}
